...
const Dashboard = () => {
  ...
  const handleFile = async (f: File) => {
    ...
    if (isBinary) {
      toast.info("PDF/DOC detected — please paste your resume text below for best accuracy.", { duration: 6000 });
      return;
    }
    try {
  ...
  const downloadEnhancedResume = () => {
    ...
    const html = `<!DOCTYPE html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8"><title>Enhanced Resume</title><style>
body{font-family:Calibri,Arial,sans-serif;color:#222;max-width:780px;margin:24px auto;padding:0 20px;line-height:1.5}
h1{font-size:22pt;margin:0 0 4px;border-bottom:2px solid #2563eb;padding-bottom:6px;color:#0f172a}
h2{font-size:14pt;margin:22px 0 8px;color:#2563eb;text-transform:uppercase;letter-spacing:.5px}
h3{font-size:12pt;margin:14px 0 4px;color:#0f172a}
p{margin:6px 0}
ul{margin:6px 0 12px 20px;padding:0}
li{margin:3px 0}
.tag{display:inline-block;background:#eff6ff;color:#1d4ed8;padding:2px 8px;border-radius:10px;font-size:9pt;margin:2px 4px 2px 0}
.score{font-size:11pt;color:#475569;margin-bottom:14px}
.box{border-left:3px solid #2563eb;background:#f8fafc;padding:8px 12px;margin:6px 0}
.section-tag{background:#dbeafe;color:#1e40af;padding:1px 8px;border-radius:8px;font-size:9pt;font-weight:600}
small{color:#64748b;font-size:9pt}
</style></head><body>
<h1>Enhanced Resume</h1>
<div class="score">JD Alignment Score: <strong>${enhancement.alignment_score}/100</strong> &nbsp;•&nbsp; Generated by Skillora AI</div>

<h2>Professional Summary</h2>
<p>${safe(enhancement.rewritten_summary)}</p>

<h2>Overall Assessment</h2>
<p>${safe(enhancement.overall_summary)}</p>

<h2>Strengths</h2>
<ul>${enhancement.strengths.map((s) => `<li>${safe(s)}</li>`).join("")}</ul>

<h2>Areas to Improve</h2>
<ul>${enhancement.weaknesses.map((s) => `<li>${safe(s)}</li>`).join("")}</ul>

${enhancement.missing_keywords.length ? `<h2>Keywords to Add (ATS)</h2><p>${enhancement.missing_keywords.map((k) => `<span class="tag">${safe(k)}</span>`).join("")}</p>` : ""}

<h2>Suggested Changes</h2>
${enhancement.suggested_changes
  .map(
    (c) => `<div class="box">
      <h3><span class="section-tag">${safe(c.section)}</span></h3>
      <p><strong>Issue:</strong> ${safe(c.issue)}</p>
      <p><strong>Suggestion:</strong> ${safe(c.suggestion)}</p>
      ${c.example ? `<p><strong>Example:</strong> <em>${safe(c.example)}</em></p>` : ""}
    </div>`,
  )
  .join("")}

<p style="margin-top:30px"><small>Apply these changes to your existing resume in Word/Google Docs to keep your formatting.</small></p>
</body></html>`;
    const blob = new Blob(["\ufeff", html], { type: "application/msword" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `enhanced-resume-${Date.now()}.doc`;
