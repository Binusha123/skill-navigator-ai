...
const systemPrompts: Record<Step, string> = {
  ...
  evaluate_answers: "You are an expert evaluator. Return EXACTLY ONE evaluation entry per UNIQUE required skill in `required_skills` — never duplicate the same skill name. If multiple questions in `qa` map to the same skill, consolidate them into a single averaged judgement. Each entry must have a DISTINCT score (do not give every skill the same score) reflecting the candidate's actual demonstrated proficiency for that specific skill (0-100), the role's required threshold (0-100), and a SHORT skill-specific feedback sentence that references what the candidate did well or poorly for THAT skill (not a generic line). If no answer was given for a skill, score it low (10-30) and say so.",
